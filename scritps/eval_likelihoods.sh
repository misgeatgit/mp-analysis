...
for fd in $(seq 1 $Kfd); do
    ...
    for p in $(uniq_prefixes "$exp_dir/anal/fd_$fds" "_cnd_*.combo"); do
        CMD=" /home/addis-ai/opencog_ocpkg/opencog/build/opencog/learning/moses/main/eval-candidate "
        CMD+=" -i $ifile"
        CMD+=" -u OUT"  #my modification(Misgana) changed value of -u from out to f280
        CMD+=" -H prerec"
        ...
        ofile="${p}.weights"
        CMD+=" -o $ofile"
        #echo "$CMD"  #my modification(Misgana)
        $CMD          #my modification(Misgana) this and added line
    done
done | $PPAR
