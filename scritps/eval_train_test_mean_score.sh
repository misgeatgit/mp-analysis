...
for p in $(uniq_prefixes "$exp_dir/anal/" "_cnd_*_train.csv"); do
    ...
    for sc in precision recall; do
        for cnd in $(seq 0 $(($candidates - 1))); do
            ...
            $sc $actual_file $prediction_file OUT #my modification here f280 instead of out
...
for p in $(uniq_prefixes "$exp_dir/anal/" "_combined.csv"); do
    ...
    for sc in precision recall; do
        ...
        $sc $actual_file $prediction_file OUT > ${p}_combined.${sc} #my modification here f280 instead of out
    done
done
