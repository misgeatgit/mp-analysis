...
if [[ $# != 2 ]]; then
    ...
    echo "Usage: $0 SETTINGS_FILE DATASET_FILE"    
    exit 1
fi
...
ei=0
CMD=" /home/addis-ai/opencog_ocpkg/opencog/build/opencog/learning/moses/main/moses "
for fd in $(seq 1 $Kfd); do
    ...
    #for the arrhytmia decilized one
    GPO+=" -u OUT "
    ...
    for nfeats in ${nfeats_seq[@]}; do
        for conf in ${conf_seq[@]}; do
            ...
            FCMD=" /home/addis-ai/opencog_ocpkg/opencog/build/opencog/learning/feature-selection/main/feature-selection "
                        ...
                        for fsm_scorer in ${fsm_scorer_seq[@]}; do
                            ...
                            if [[ $fsm_algo == smd ]]; then
                               ...
                               # FSPO+=" --fs-mi-penalty -$fsm_conf"
                                  FSPO+=" --fs-mi-penalty $fsm_conf" #my modification
